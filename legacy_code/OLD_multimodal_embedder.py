...
class ColPaliEmbedder(BaseEmbedder):
    ...
    def __init__(self, model_name="vidore/colpali-v1.3", device=None):
        ...
        self.model = ColPali.from_pretrained(
            model_name,
            torch_dtype=torch.float32,  # Use float32 for CPU (bfloat16 is GPU-optimized)
            device_map=None,  # Ensures everything is loaded into CPU
            offload_folder=None,  # Prevents disk offloading
            low_cpu_mem_usage=False  # Ensures full CPU loading instead of memory mapping
        ).eval()
